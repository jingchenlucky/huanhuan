...
- 不依赖宿主环境：React只依赖原生JS语言，不依赖任何其他东西，包括运行环境。因此，它可以被轻松的移植到浏览器、桌面应用、移动端。(平时用的vue依赖宿主环境，只用于网页，后来进行了拆分vue核心库，不依赖于宿主环境。所以在移动端，react发展更快)
- 渐近式：React并非框架，对整个工程没有强制约束力(框架有侵入性，vue框架，要求用户必须要怎么样，例如路由和vuex)。这对与那些已存在的工程，可以逐步的将其改造为React，而不需要全盘重写（React和JQ可以同时用）。
- 单向数据流：所有的数据自顶而下的流动
- 用JS代码声明界面（vue用template声明界面，更像是一个html 文件）
- 组件化
